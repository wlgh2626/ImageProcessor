...
import com.project.image.tiff.header.Properties.FileType;
...
public class Header {
	...
	public Header(byte[] header) throws Exception{	// Read the 8 bytes long TIFF header 
		order = Arrays.equals(ByteReader.INSTANCE.read(header, 0, 2), BIG_ENDIAN_VALUE) ? 
				ByteOrder.BIG_ENDIAN : 
				ByteOrder.LITTLE_ENDIAN;
		
		ByteReader.INSTANCE.setByteOrder(order);	//Set all reads to Big/Little Endian
		fileType = Arrays.equals(ByteReader.INSTANCE.read(header, 2, 4), TIFF_VALUE) ? 
				FileType.TIFF : 
				FileType.UNKNOWN;
		
		if(!fileType.equals(FileType.TIFF)) throw new Exception("The given FileType is " + fileType + ". Only TIFF file is accepted");
